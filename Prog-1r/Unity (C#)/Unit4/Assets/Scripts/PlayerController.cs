...
public class PlayerController : MonoBehaviour {
    ...
    private enum SmashState {None,Up,Down};
    ...
    void Update() {
        if (_smashState == SmashState.None) {
            ...
            Vector3 movement = _focalPoint.transform.forward * forwardInput + _focalPoint.transform.right * horizontalInput;
    ...
    private IEnumerator PowerupSmushRoutine() {
        for (int i = 0; i < 3; i++) {
            _smashPosition = transform.position;
    ...
    private void SmashEnemies() {
        ...
        foreach (var enemy in enemies) {
            ForceEnemy(enemy, _smashForce, distanceDependent: true); } }
